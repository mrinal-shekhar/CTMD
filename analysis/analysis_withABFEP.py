...
N_BOOTSTRAP=360
DOCK_LABEL="Glide XP"
SETUP_FILE = "publication_data/JAK2/hit_data_mol2.spc" # Defines path to base directory and the list of molecules
ABFEP_SCORES="publication_data/JAK2/JAK2_ABFEPscore.csv"
BASE_DIR=SETUP_FILE[:SETUP_FILE.rfind("/")+1]
#BOLTZ2_FILE="publication_data/JAK2/JAK2_boltz_prediction.csv"
BOLTZ2_FILE="publication_data/JAK2/JAK2_phe_mut_boltz_prediction.csv" # Active site mutated to PHE
#BOLTZ2_FILE="publication_data/JAK2/JAK2_ala_mut_boltz_prediction.csv" # Active site mutated to ALA
#BOLTZ2_FILE="publication_data/JAK2/JAK2_flip_mut_boltz_prediction.csv" # For boltz2 with mutations (charged residues flipped charges)
...
# ABFEP
setupfile_abfep=CTMDDatabase(SETUP_FILE,allow_load=False)
score_data=pd.read_csv(ABFEP_SCORES)
abfep_scores=(score_data["ABFEP run1"]+score_data["ABFEP run2"]+score_data["ABFEP run3"]).to_numpy().astype(float)/3
ligand_names=score_data["Lig"]
abfep_dict=generate_dict(ligand_names,abfep_scores)
setupfile_abfep.hardcode_scores(abfep_dict)

# Glide XP Score
setupfile_xp=CTMDDatabase(SETUP_FILE,allow_load=False)
score_data=pd.read_csv(ABFEP_SCORES)
xp_scores=score_data["Glide XP score"].to_numpy().astype(float)
ligand_names=score_data["Lig"]
...
bz_scores=score_data["affinity"].to_numpy().astype(float)
#bz_scores=1/(score_data["probability"].to_numpy().astype(float)) # Both give very similar ranking
...
for _ in tqdm.tqdm(range(N_BOOTSTRAP),ncols=100):
    try:
        ...
        # ABFEP
        ligand_scores=np.array([setupfile_abfep.getBootstrappedCTMDScore(str(k))[0] for k in selected_ligands],dtype=float)
        n_ab,abfep_hit_rates=get_enrichment_factor_trend(np.array([setupfile_abfep.getClass(k) for k in selected_ligands],dtype=bool),-ligand_scores[:,0])
        n_abfep_total.append(n_ab)
        hit_rate_abfep_total.append(abfep_hit_rates)
...
n_abfep_total=np.array(n_abfep_total)
hit_rate_abfep_total=np.array(hit_rate_abfep_total)
sN["XPScore"]=np.array(sN["XPScore"])
sH["XPScore"]=np.array(sH["XPScore"])
sN["Boltz2"]=np.array(sN["Boltz2"])
sH["Boltz2"]=np.array(sH["Boltz2"])
...
# ABFEP
xdata=np.mean(n_abfep_total,axis=0)
y_mean=np.mean(hit_rate_abfep_total,axis=0)
y_err=np.std(hit_rate_abfep_total,axis=0)
plt.errorbar(xdata+xshift,(y_mean/BASE_HIT_RATE),yerr=(y_err/BASE_HIT_RATE),color="red",linewidth=3,label="ABFEP")

ilocx=np.argmin(np.abs(xdata-TARGET_FRACTION))
print(f"ABFEP Enrichment factor at screened fraction {TARGET_FRACTION}:",xdata[ilocx],y_mean[ilocx]/BASE_HIT_RATE,"+/-",y_err[ilocx]/BASE_HIT_RATE,flush=True)

# XPScore
xdata=np.mean(sN["XPScore"],axis=0)
y_mean=np.mean(sH["XPScore"],axis=0)
y_err=np.std(sH["XPScore"],axis=0)
#plt.errorbar(xdata+xshift*2,(y_mean/BASE_HIT_RATE),yerr=(y_err/BASE_HIT_RATE),color="blue",linewidth=3,label=DOCK_LABEL) # Uncomment to include Glide Scores
...
y_mean=np.mean(hit_rate_abfep_total,axis=0)
y_err=np.std(hit_rate_abfep_total,axis=0)
alphas_ABFEP,bedroc_ABFEP=getBEDROCProfile(y_mean)
plt.errorbar([0.]+list(xdata),[0.]+list((y_mean*np.arange(1,len(xdata)+1))/N_HIT),yerr=[0.]+list((y_err*np.arange(1,len(xdata)+1))/N_HIT),color="red",linewidth=3,label="ABFEP")
...
#plt.errorbar([0.]+list(xdata),[0.]+list((y_mean*np.arange(1,len(xdata)+1))/N_HIT),yerr=[0.]+list((y_err*np.arange(1,len(xdata)+1))/N_HIT),color="blue",linewidth=3,label=DOCK_LABEL) # Uncomment to include GLIDE Scores
...
plt.plot(alphas_ABFEP,bedroc_ABFEP,color="red",linewidth=3,label="ABFEP")
#plt.plot(alphas_xp,bedroc_xp,color="blue",linewidth=3,label=DOCK_LABEL) # Un-comment to include glide scores in plot
...
np.save(BASE_DIR+"/BEDROC_data.npy",(alphas_CTMD,bedroc_CTMD,bedroc_ABFEP,bedroc_xp,bedroc_bz)) # CTMD, ABFEP, DOCK, Boltz-2
